...
def apply_background_subtraction_to_array(
    ...
    # Convert back to BGR for display if original was color
    if len(frame.shape) == 3:
            ...
            if grabResult.GrabSucceeded():
                try:
                    ...
                    if bg_subtraction_enabled and not show_original:
                        processed_img = apply_background_subtraction_to_array(
                ...
                print(f"Grab failed: {grabResult.GetErrorDescription()}")
            grabResult.Release()
...
def main():
    ...
    try:
        # Step 1: Record background video
        output_dir = create_output_directory(os.path.dirname(__file__))
        ...
        if not success:
            ...
            # Step 2: Extract background from video
