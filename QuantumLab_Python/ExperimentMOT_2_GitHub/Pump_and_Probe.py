...
#%% FOLDER REFERENCE
folder_path = r'C:\Users\MOT_USER\Documents\Python Scripts\QuantumLabPython\ExperimentMOT_special_2\Output' + '\\' + 'Pump_and_Probe'
...
No_MOT()
time.sleep(1.5)
### Probe bkg always before because it does not control its detuning
Bkg_probe_ctrl = Background_capture(MeasType = 'Probe', Camera = 'Cam0', Exposure = 150, folder_path = folder_path)
Bkg_pump_ctrl = Background_capture(MeasType = 'Pump', Camera = 'Cam1', Exposure = 50, folder_path = folder_path)
...
time.sleep(0.1)
init_detuning = 3.75
DS_AWG1.ApplyDCVoltage(Load ='1000', AWGChannelNum = '2', Volt = str(init_detuning))
...
number_of_experiments = 100 ### Number of experiments performed
AWGChannelsToBeUsed = ['AWG1_1', 'AWG2_1', 'AWG3_1', 'AWG3_2', 'AWG4_1', 'AWG5_1', 'AWG5_2']
...
WaveformList, Headers = CreateArbitraryWaveformVectorFromCSVFile('Pump_and_Probe_B_off.csv', RowNumber = 10)
...
if 'AWG3_1' in AWGChannelsToBeUsed:
    ...
    DS_AWG3.SetBurstOuputArbitraryWaveform('1000', FuncName = FunctionName, AWGChannelNum = '1', SampleRate = '100000', VHigh = '9', VLow = '0')
...
if 'AWG4_1' in AWGChannelsToBeUsed:
    ...
    DS_AWG4.SetBurstOuputArbitraryWaveform('50', FuncName = FunctionName, AWGChannelNum = '1', SampleRate = '100000', VHigh = '5', VLow = '0')
    
if 'AWG5_1' in AWGChannelsToBeUsed:
    FunctionName = Mg.ResourceNameToJob['AWG5_1']
    FunctionVector = SelectWaveform(Headers, WaveformList, FunctionName)
    DS_AWG5.AddArbitraryWaveformToChannelVolatileMemory(FunctionVector, AWGChannelNum = '1', FuncName = FunctionName)
    DS_AWG5.SetBurstOuputArbitraryWaveform('INF', FuncName = FunctionName, AWGChannelNum = '1', SampleRate = '100000', VHigh = '5', VLow = '0')
...
try: 
    ...
    if 'Cam1' in ListOfCamerasToBeTriggered:
        ...
        MCS.Set_ROI('Cam1', CamBinning = 1, PixelWidth = 384, PixelHeight = 384, OffX = 940, OffY = 780)    
...
# %% TRIGGER EXPERIMENT  
### The MOT should be destroied before moving to a different detuning.
if TRG == 'y':
    print('START EXPERIMENTS: ', '\n')
    try: 
        list_of_detunings = []
        for det in tqdm(np.arange(0, 4, 0.25)):       
            det_value = init_detuning + det
            DS_AWG1.ApplyDCVoltage(Load ='1000', AWGChannelNum = '2', Volt = str(det_value))
            print('MOT detuning [V]: ', str(det_value))
            if 'AWG1_1' in AWGChannelsToBeUsed:
                FunctionName = Mg.ResourceNameToJob['AWG1_1']
                FunctionVector = SelectWaveform(Headers, WaveformList, FunctionName)
                DS_AWG1.AddArbitraryWaveformToChannelVolatileMemory(FunctionVector, 
                                                                    AWGChannelNum = '1', FuncName = FunctionName)
                DS_AWG1.SetBurstOuputArbitraryWaveform('1000', FuncName = FunctionName, 
                                                       AWGChannelNum = '1', SampleRate = '100000', VHigh = '9', 
                                                       VLow = '0')            
            time.sleep(6) ### LOAD           
            ...
            for i in range(number_of_experiments): 
                ...
                random_sleep = (round(random.random(), 3))/2
                time.sleep(random_sleep) ### RANDOM PRE-LOAD
                print('Random loading interval [s]: ', '\n', str(random_sleep))
                ###--------------------------------------
...
if TRG_performed == 'y':                    
    Bkg_pump_mx = Image_Matrix(ImageName = 'Cam1_0_1.bmp', folder_path = folder_path + '\\' + 'Background')    
    Bkg_pump_dark_mx = Image_Matrix(ImageName = 'Cam1_0_0.bmp', folder_path = folder_path + '\\' + 'Background')                 
    ...
    for i in range(0, 16, 1):                   
        ### Pump
        Abs_pump_mx = Image_Matrix(ImageName = 'Cam1_%d_0_0.bmp' %i, folder_path = folder_path)
        Pump_abs_I = SubtractImgs(SubtractImgs(Bkg_pump_mx.image, Bkg_pump_dark_mx.image), 
                                  SubtractImgs(Abs_pump_mx.image, Bkg_pump_dark_mx.image))                                           
        ### Probe
...
''' Optical Density vs. number of experiments to check if the loading time is correct. To compare with the
random loading you find in 'output.txt' file, which is the log of the experiment.
...
if TRG_performed == 'y':
    ...
    Bkg_probe_mx = Image_Matrix(ImageName = 'Cam0_0_1.bmp', folder_path = folder_path + '\\' + 'Background')   
    Bkg_probe_dark_mx = Image_Matrix(ImageName = 'Cam0_0_0.bmp', folder_path = folder_path + '\\' + 'Background')   
    Probe_imgs = [list_of_detunings[5][i]['Cam0'][0] for i in range(number_of_experiments)]
