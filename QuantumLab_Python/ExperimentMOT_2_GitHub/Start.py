...
from AnalysysBMP_Exp import Gauss, FlatTopGauss, SubtractImgs, DivideImgs, PanShotAbsorption, PanShotAbsorptionReduced, Image_Matrix
...
def No_MOT():
    if 'AWG1' in Mg.OpenedResourceNames: DS_AWG1.ApplyDCVoltage(Load ='1000', AWGChannelNum = '1', Volt = '9')   
    if 'AWG2' in Mg.OpenedResourceNames: DS_AWG2.ApplyDCVoltage(Load ='1000', AWGChannelNum = '1', Volt = '0')
    if 'AWG5' in Mg.OpenedResourceNames: DS_AWG5.ApplyDCVoltage(Load ='INF', AWGChannelNum = '1', Volt = '0')
    return None
...
def Background_capture(MeasType, Camera, Exposure, folder_path):
    ...
    '''
    if Exposure <= 50: exp_cam = 50
    else: exp_cam = Exposure
    ...
    if MeasType == 'Scattering':
        ModifyCSV(FileName = 'Background.csv', device = 'MOT_switch', start = 31, 
                  exposure = Exposure//10, value = 0.111)
        ModifyCSV(FileName = 'Background.csv', device = 'Rep_switch', start = 31, 
                  exposure = Exposure//10, value = 1)       
    if MeasType == 'Pump' :
        ModifyCSV(FileName = 'Background.csv', device = 'MOT_switch', start = 31, 
                  exposure = Exposure//10, value = 1)
        ModifyCSV(FileName = 'Background.csv', device = 'Rep_switch', start = 31, 
                  exposure = Exposure//10, value = 0) 
        ModifyCSV(FileName = 'Background.csv', device = 'AWG4_1', start = 1051, 
                  exposure = Exposure//10, value = 1) 
        ModifyCSV(FileName = 'Background.csv', device = 'Probe_2pass', start = 1050, 
                  exposure = Exposure//10 + 1, value = 0.444) 
    if MeasType == 'Probe':
        ModifyCSV(FileName = 'Background.csv', device = 'MOT_switch', start = 31, 
                  exposure = Exposure//10, value = 1)
        ModifyCSV(FileName = 'Background.csv', device = 'Rep_switch', start = 31, 
                  exposure = Exposure//10, value = 0)  
        ModifyCSV(FileName = 'Background.csv', device = 'Probe_switch', start = 1051, 
                  exposure = Exposure//10, value = 0.111) 
        
    ### PREPARATION
    WaveformList, Headers = CreateArbitraryWaveformVectorFromCSVFile('Background.csv', RowNumber = 10)
    ...
    try: 
        ...
        if 'Cam0' in ListOfCamerasToBeTriggered:
            ...
            MCS.Set_ROI('Cam0',  CamBinning = 1, PixelWidth = 208, PixelHeight = 208, OffX = 920, OffY = 880) 
        ...
        if 'Cam1' in ListOfCamerasToBeTriggered:
            ...
            MCS.Set_ROI('Cam1', CamBinning = 1, PixelWidth = 384, PixelHeight = 384, OffX = 870, OffY = 800)    
        ...
        for j in range(number_of_experiments):
            for i in ListOfCamerasToBeTriggered:
                for k in range(CamNameToPicNum[i]):
                    ...
                    img_name_tosave = folder_path + '\\' + 'Background' + '\\' + i + '_' + str(j) + '_' + str(k) + '.bmp'
    ...
    if TRG_performed == 'y':
        if MeasType == 'Probe':
            Bkg_probe_mx = Image_Matrix(ImageName = Camera + '_0_1.bmp', 
                                        folder_path = folder_path + '\\' + 'Background')
            Bkg_probe_dark_mx = Image_Matrix(ImageName = Camera + '_0_0.bmp', 
                                             folder_path = folder_path + '\\' + 'Background')
            plt.figure() 
            plt.subplot(121)
            plt.tight_layout()
    ...
    if TRG_performed == 'y':   
        if MeasType == 'Pump':
            Bkg_pump_mx = Image_Matrix(ImageName = Camera + '_0_1.bmp', 
                                       folder_path = folder_path + '\\' + 'Background')
            Bkg_pump_dark_mx = Image_Matrix(ImageName = Camera + '_0_0.bmp', 
                                            folder_path = folder_path + '\\' + 'Background')
            plt.figure() 
            plt.subplot(121)
            plt.tight_layout()
