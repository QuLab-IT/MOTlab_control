...
Here we determine the detuning of the pump beam.
The mot is produced always at the same detuning and then hit
by the pump laser for 20 seconds (no rod in the laser beam path).
The pump laser detuning is changed at every experiment.
What is left of the mot after the pumping is probed with the probe beam.
The pump detuning at which the optical density is less is the best choice 
for the experiment.
...
#%% FOLDER REFERENCE
folder_path = r'C:\Users\MOT_USER\Documents\Python Scripts\QuantumLabPython\ExperimentMOT_special_2\Output\Pump_detuning' 
...
WaveformList, Headers = CreateArbitraryWaveformVectorFromCSVFile('Pump_detuning.csv', RowNumber = 10)
...
if TRG == 'y':
    ...
    try: 
        ...
        for det in tqdm(np.arange(0, 4, 0.25)):       
            det_value = 5 + det
            print('Pump detuning [V]: ', str(det_value))
            DS_AWG4.ApplyDCVoltage(Load ='1000', AWGChannelNum = '2', Volt = str(det_value))        
