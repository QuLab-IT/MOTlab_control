...
from pylablib.devices.AWG import Agilent33500
...
def CreateArbitraryWaveformVectorFromCSVFile(FileName, RowNumber=1):
    ...
    RULES:
    1) If you are using Excel to create the .csv always save in CSV (MS-DOS).
    ...
    of the uploaded waveform and output it.
    """
    Data = []
    WaveVect = []
    WaveVectors = []
...
def SelectWaveform(Headers, WaveVectors, VectName):
    ...
    Wavevetors and Header are the ouptuts of
...
class ResourceManagerCreator:
    ...
    def WhoIsUp(self):
        ...
        if self.OpenedResources != 0:
            ...
            print("There are no open resources", "\n")
...
class ResourceSession:
    ...
    def __init__(self, ResMgCrt, ResourceName):
        ...
        self.resource = ResMgCrt.rm.open_resource(
            resource_name=ResMgCrt.ResourceNameToResourceIdentityString[ResourceName]
        )  ### self.resource is a resource
        ...
        self.resource_identity_string = ResMgCrt.ResourceNameToResourceIdentityString[
            ResourceName
        ]
...
class OscilloscopeSession(ResourceSession):
    ...
    def read_data_single_channel(self, RecordLength=10000):
        ...
        self.ymult = float(
            self.resource.query("WFMOutpre:YMUlt?")
        )  # waveform vertical scale factor
        self.yzero = float(
            self.resource.query("WFMOutpre:YZERO?")
        )  # waveform vertical zero
        self.yoff = float(
            self.resource.query("WFMOutpre:YOFf?")
        )  # waveform vertical position
        self.xincr = float(
            self.resource.query("WFMOutpre:XINC?")
        )  # waveform horizontal sampling interval
...
class AWGSession(ResourceSession):
    ...
    def __init__(self, ResMgCrt, ResourceName, role="Gunner"):
        ...
        self.role = role
        self.triggersource = ""
        self.awg = Agilent33500(self.resource_identity_string)
        if self.role == "Captain":
            self.triggersource = "BUS"
            self.awg.enable_trigger_output("ON")
        else:
            self.triggersource = "EXT"
            self.awg.enable_trigger_output("OFF")
        print("Resource role: " + self.role)
        if self.resource.query("OUTP1?"):
            self.awg.enable_output("OFF", channel=1)
    ...
    def ApplyBuiltinWaveform(
        self, Load, AWGChannelNum, FunctionType="SIN", Freq="1e3", Vpp="1", Offset="0"
    ):
    ...
    def ApplyDCVoltage(self, Load, AWGChannelNum, Volt="0"):
        ...
        self.resource.write(
            "SOUR" + AWGChannelNum + ":" + "APPL" + ":" + "DC DEF, DEF, " + Volt
        )
    ...
    def OutputOff(self, AWGChannelNum):
        ...
        print(
            ...
            + AWGChannelNum
            + ": "
            + self.resource.query("OUTP" + AWGChannelNum + "?")
        )
    ...
    def ClearVolatileMemory(self, AWGChannelNum):
        """Clear event register, error queue -when power is cycled-."""
        self.resource.write(
            "SOUR" + AWGChannelNum + ":" + "DATA:VOL:CLE"
    ...
    def SyncOff(self):
        ...
        print(
            ...
            + self.resource_name
            + ": "
            + self.resource.query("OUTP:SYNC?")
            + " \n"
        )

    def AddArbitraryWaveformToChannelVolatileMemory(
        self, FuncVect, AWGChannelNum, FuncName
    ):
        ...
        self.func_vect = str()
        for i in range(1, len(FuncVect) - 1):
            self.func_vect = self.func_vect + FuncVect[i]
        self.resource.write(
            "SOUR" + AWGChannelNum + ":" + "DATA:VOL:CLE"
        )  ### Clear Volatile Memory
        ...
        self.resource.write("*WAI")  ### Wait for the operation to be completed
        self.resource.write(
            "SOUR" + AWGChannelNum + ":" + "FUNC:ARB " + FuncName
        )  ### select the desired waveform

    def ApplyArbitraryWaveform(
        self, FuncName, AWGChannelNum, SampleRate, Vpp="2.5", Offset="0"
    ):
        ...
        print(
            "ARB apply: " + self.resource.query("SOUR" + AWGChannelNum + ":" + "APPL?")
        )

    def SetBurstOuputArbitraryWaveform(
        self,
        Load,
        FuncName,
        AWGChannelNum,
        ...
        self.awg.set_load(Load, channel=AWGChannelNum)
        self.awg.set_function("arb", channel=AWGChannelNum)
        self.resource.write(
            "SOUR" + AWGChannelNum + ":" + "FUNC:ARB " + FuncName
        ...
        self.resource.write(
            "SOUR" + AWGChannelNum + ":" + "FUNC:ARB:SRAT " + SampleRate
        )  ### Sa/s
        # print('Sample Rate: ' + self.resource.query('SOUR' + AWGChannelNum + ':FUNC:ARB:SRAT?'))
        self.awg.set_output_range([float(VLow), float(VHigh)], channel=AWGChannelNum)
        self.VLow, self.VHigh = self.awg.get_output_range(channel=AWGChannelNum)
        print(
            ...
            + self.resource_name
            + ": "
            + self.resource.query("SOUR" + AWGChannelNum + ":FUNC:ARB?"),
            end="",
        )
        print(
            f"VHigh: {self.VHigh} V, VLow: {self.VLow} V, Sample Rate: {SampleRate} Sa/s, Load: {Load} Ohm\n"
        )
        self.awg.wait_dev()
        ### BURST commands.
        self.awg.set_burst_mode("trig", channel=AWGChannelNum)
        self.awg.set_burst_ncycles(1, channel=AWGChannelNum)  ### Burst cycles
        self.awg.set_trigger_source(
            self.triggersource, channel=AWGChannelNum
        )  ### BUS trigger allows to have repetition of waveform as much as NCYC
        self.awg.enable_burst(True, channel=AWGChannelNum)
        # if (float(VHigh)) < 5.2 and (float(VLow)) > -5.2:
        self.awg.enable_output(True, channel=AWGChannelNum)
        self.awg.wait_dev()
        # else:
    ...
    def Trigger(self):
        ...
        if self.role == "Captain":
            if self.resource.query("*OPC?"):
                ...
                print(
                    "Communication still running between the computer and the devices. No trigger was otputted."
                )
        else:
            print(
                "The selected device is not a Captain and cannot therefore be triggered"
            )

    def PrintError(self):
        """Print eventual errors occurred."""
        print(
            self.resource_name + " Errors: " + self.resource.query("SYST:ERR?"), end=""
        )
