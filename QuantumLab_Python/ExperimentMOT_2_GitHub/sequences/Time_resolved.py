...
from QuantumLab_Python.ExperimentMOT_2_GitHub.utils.AnalysysBMP_Exp import Image_Matrix, SubtractImgs
...
time.sleep(1.5)
###
Bkg_scatt_2_ctrl = Background_capture(
    MeasType="Scattering",
    Camera="Cam2",
    Exposure=Exposure_Cam2,
...
try:
    ...
    if "Cam2" in ListOfCamerasToBeTriggered:
        ...
        MCS.Set_ROI(
            "Cam2", CamBinning=1, PixelWidth=144, PixelHeight=144, OffX=1150, OffY=720
        )
...
if TRG_performed == "y":
    for i in ListOfCamerasToBeTriggered:
        Bkg_Cam2_mx = Image_Matrix(
            ImageName=i + "_0_0.bmp", folder_path=folder_path + "\\" + "Background"
        )
        Cam2_mx = Image_Matrix(ImageName=i + "_0_0_0.bmp", folder_path=folder_path)
        Cam2_img = SubtractImgs(Cam2_mx.image, Bkg_Cam2_mx.image)
        Cam2_last_mx = Image_Matrix(
            ImageName=i + "_0_0_%d.bmp" % (CamNameToPicNum[i] - 1),
            folder_path=folder_path,
        )
