...
#%% FOLDER REFERENCE
folder_path = r'C:\Users\MOT_USER\Documents\Python Scripts\QuantumLabPython\ExperimentMOT_special_2\Output' + '\\' + 'Probe_detuning'
...
WaveformList, Headers = CreateArbitraryWaveformVectorFromCSVFile('Probe_detuning_B_off.csv', RowNumber = 10)
...
if TRG_performed == 'y':
    ...
    for i in range(0, 16, 4):                                                             
        ...
        Bkg_dark_mx = Image_Matrix(ImageName = 'Cam0_%d_0_1.bmp' %i, folder_path = folder_path)    
        Bkg_probe_mx = Image_Matrix(ImageName = 'Cam0_%d_0_2.bmp' %i, folder_path = folder_path)
        Probe_abs_I = SubtractImgs(SubtractImgs(Bkg_probe_mx.image, Bkg_dark_mx.image), SubtractImgs(Abs_probe_mx.image, Bkg_dark_mx.image))
        Probe_opt = LogImg(DivideImgs(SubtractImgs(Bkg_probe_mx.image, Bkg_dark_mx.image), SubtractImgs(Abs_probe_mx.image, Bkg_dark_mx.image)))
        ### Plot
        plt.figure()    
        plt.subplot(131)
        plt.tight_layout()
...
if TRG_performed == 'y': 
    ...
    for j in range(0, 16, 1):                
        ...
        Bkg_dark_imgs = [list_of_detunings[j][i]['Cam0'][1] for i in range(number_of_experiments)]
        Bkg_probe_imgs = [list_of_detunings[j][i]['Cam0'][2] for i in range(number_of_experiments)]   
        
        Probe_opt_imgs = [LogImg(DivideImgs(SubtractImgs(Bkg_probe_imgs[i], Bkg_dark_imgs[i]), SubtractImgs(Probe_imgs[i], Bkg_dark_imgs[i]))) for i in range(number_of_experiments)]
    ...
    plt.errorbar([i for i in np.arange(5, 9, 0.25)], Exp_mean_list, fmt = 'o', yerr=Exp_std_list, xerr=None)
    #plt.yscale('log')
    plt.grid()
    plt.show(block = False)
...
#%% Release Memory
if TRG == 'y':
    del list_of_detunings, list_of_dictionaries, Probe_imgs, Bkg_dark_imgs, Bkg_probe_imgs, Probe_opt_imgs, Px_list
    gc.collect()
