...
#%% FOLDER REFERENCE
folder_path = r'C:\Users\MOT_USER\Documents\Python Scripts\QuantumLabPython\ExperimentMOT_special_2\Output' +\
            '\\' + 'MOT_detuning'
Wait = 300   ### Wait before scattering measurement, after unloading the MOT, us 
Exposure = 180 ### us

ModifyCSV(FileName = 'MOT_detuning.csv', device = 'MOT_switch', start = 31 +\
          Wait//10, exposure = Exposure//10, value = 0.111)   
ModifyCSV(FileName = 'MOT_detuning.csv', device = 'AWG5_2', start = 31, exposure = Wait//10, value = 1)  
...
time.sleep(1.5)
###
Bkg_Cam2_ctrl = Background_capture(MeasType = 'Scattering', Camera = 'Cam2', 
                                   Exposure = Exposure , folder_path = folder_path)
AWGBaseConfiguration()
...
'''
WaveformList, Headers = CreateArbitraryWaveformVectorFromCSVFile('MOT_detuning.csv', RowNumber = 10)
...
if 'AWG1_2' in AWGChannelsToBeUsed:
    ...
    DS_AWG1.SetBurstOuputArbitraryWaveform('1000', FuncName = FunctionName, AWGChannelNum = '2', SampleRate = '100000', VHigh = '9', VLow = '0')
...
try: 
    ...
    if 'Cam2' in ListOfCamerasToBeTriggered:
        ...
        MCS.Set_ROI('Cam2', CamBinning = 1, PixelWidth = 144, PixelHeight = 144, OffX = 1170, OffY = 710)
...
if TRG == 'y':
    ...
    try:
        ...
        for det in tqdm(np.arange(0, 4, 0.25)):  
            ...
            ModifyCSV(FileName = 'MOT_detuning.csv', device = 'MOT_2pass', start = 31, exposure = Exposure//10 +\
                      Wait//10, value = round(det_value/9, 3))        
            WaveformList, Headers = CreateArbitraryWaveformVectorFromCSVFile('MOT_detuning.csv', RowNumber = 10)
            if 'AWG1_2' in AWGChannelsToBeUsed:
                DS_AWG1.ClearVolatileMemory('2')
                FunctionName = Mg.ResourceNameToJob['AWG1_2']
                FunctionVector = SelectWaveform(Headers, WaveformList, FunctionName)
                DS_AWG1.AddArbitraryWaveformToChannelVolatileMemory(FunctionVector, AWGChannelNum = '2', 
                                                                    FuncName = FunctionName)
                DS_AWG1.SetBurstOuputArbitraryWaveform('1000', FuncName = FunctionName, AWGChannelNum = '2', 
                                                       SampleRate = '100000', VHigh = '9', VLow = '0')   
            ###------------------------------------------------------------
...
ModifyCSV(FileName = 'MOT_detuning.csv', device = 'MOT_switch', start = 31 +\
          Wait//10, exposure = Exposure//10, value = 1)   
ModifyCSV(FileName = 'MOT_detuning.csv', device = 'AWG5_2', start = 31, exposure = Exposure//10 +\
          Wait//10, value = 0)    
ModifyCSV(FileName = 'MOT_detuning.csv', device = 'MOT_2pass', start = 31, exposure = Exposure//10 +\
          Wait//10, value = 0.444)                             
...
if TRG_performed == 'y':  
    index_plot = 231                                    
    Bkg_Cam2_mx = Image_Matrix(ImageName = 'Cam2_0_0.bmp', folder_path = folder_path + '\\' + 'Background')  
    plt.figure()
...
Data = [] 
with open(r'C:\Users\MOT_USER\Documents\Python Scripts\QuantumLabPython\ExperimentMOT_special_2\MOT_AOM_curve.csv', 'r') as file:
    j = 1
...
'''  
if TRG_performed == 'y':                   
    Bkg_Cam2_mx = Image_Matrix(ImageName = 'Cam2_0_0.bmp', folder_path = folder_path +\
                               '\\' + 'Background')
    Exp_mean_Cam2_list = []
    Exp_std_Cam2_list = []
    
    for j in range(0, 16, 1):
        Cam2_imgs = [(SubtractImgs(list_of_detunings[j][i]['Cam2'][0], Bkg_Cam2_mx.image)) \
                     for i in range(number_of_experiments)] 
        Px_Cam2_list = [[Cam2_imgs[i][m,n] for m in range(0, Bkg_Cam2_mx.image.shape[0]) \
                         for n in range(0, Bkg_Cam2_mx.image.shape[1])] for i in range(number_of_experiments)]
        Px_sum_Cam2_list = [sum(Px_Cam2_list[i])/1000 for i in range(number_of_experiments)]
    ...
    plt.errorbar([i for i in np.arange(5, 9, 0.25)], Exp_mean_Cam2_list, fmt = 'o', yerr=Exp_std_Cam2_list, xerr=None)
    plt.grid()  
    plt.subplot(122)
    plt.tight_layout()
    plt.title('Intensity Cam2 Normalised')
    plt.xlabel('Detuning [V]')
    plt.errorbar(detunings_reduced, AOM_norm_Cam2_array, fmt = 'o', yerr=AOM_norm_Cam2_std_array, xerr=None)
    plt.grid()
    plt.show(block = False)
